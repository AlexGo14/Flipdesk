...
//Check for log directory. Create directory if not available.
var logDir = './logs';
...
logger.setLevel(nconf.get('debug').log4js.level);
...
if (cluster.isMaster) {
  ...
  cluster.on('exit', function(worker, code, signal) {
	...
	// Start E-Mail-Listener only on master
	var mail_package = require('./packages/mail');
	mail_package.startAll();
